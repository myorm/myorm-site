...
const ctx: MyORMContext<Track> = new MyORMContext(connection, "Track"));
ctx.whereNot(m => m.TrackId.equals(20))
    .whereNot(m => m.Name.equals("AC/DC"))
    .select();`;

    const sequentialSqlCode = ``;
</script>

<h2 class="h2">.whereNot()</h2>
<div class="p-4">
    <Docs>
        Apply negated filter conditions to your next transaction.
        <svelte:fragment slot="parameters">
            <Argument name="modelCallback" type="WhereCallback">
                Callback that uses the <code class="code">model</code> parameter to get reference to the properties being filtered on.
            </Argument>
        </svelte:fragment>
